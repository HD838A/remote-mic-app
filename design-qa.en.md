...
- Action Sequences and Button Profiles use separate sidebar entries. Button Profiles use a three-step in-page flow: setup, buttons, and review; Return performs the primary action on every step. Only the button step consumes physical-button presses for selecting a position; the overview and other steps continue executing the saved action. Saved shortcuts are presented as a separate flat category, can be recorded inline for reuse, and can cancel recording without leaving the profile. When app links exist but manual mode is active, the overview states that automatic switching is paused and provides an inline restore action.
...
- Wizard steps must represent meaningful user tasks; a simple name field must not occupy a step by itself. The primary action on every step should support Return. While a temporary subtask such as shortcut recording is active, Return and Cancel must apply to that subtask instead of advancing or dismissing the whole wizard.
- Behavior-defining content such as shortcut chords and routine names must remain fully readable. Increase card height and wrap when needed; do not ellipsize modifiers, arrow keys, or other details that change the meaning.
