...
- Action Sequences and Button Profiles use separate sidebar entries. Button Profiles use a four-step in-page flow: name, usage, buttons, and review; Return advances from a valid name. Only the button step consumes physical-button presses for selecting a position; the overview and other steps continue executing the saved action. Saved shortcuts are presented as a separate flat category and can be recorded inline for reuse. When app links exist but manual mode is active, the overview states that automatic switching is paused and provides an inline restore action.
