...
- [completed] 9. 将微信语音动作移入微信应用专属区域
- [completed] 10. 取消微信语音动作的电源键限制
- [completed] 11. 新增微信收到语音消息的播放动作并完成实测
...
- 微信发语音消息仅在微信应用 profile 的应用专属动作区域显示，同时保留电源键和按住说话限制。
- 微信发语音消息可绑定微信 profile 中任意遥控器按键，按住/松开仍分别发送 Right Option keyDown/keyUp。
- 微信收到的语音消息可通过微信 profile 的专属动作触发播放；该动作不绑定电源键、不影响语音转文字。
