...
组合动作需要在无线麦SayAll.app 中配置、测试并绑定遥控器。它是免费功能，官方构建默认集成，但实现仍通过独立 Swift Package 与公开宿主解耦。

## 用户功能介绍

包含免费组合动作 Package 的 App 会直接显示“组合动作”侧边栏，不需要邀请码，也不会显示组合动作邀请码录入区域。用户可以创建多步骤动作、录入快捷键、学习输入框、按 Identifier 运行本机快捷指令、使用默认浏览器打开指定网址、复用已有组合动作，并绑定到遥控器的单击、双击或长按。Package 缺失时不显示入口，原有按键映射继续工作。

## 范围与非目标

- 公开仓库只维护 Swift Package 接入、页面委托、按键事件转发和缺少 Package 时的兼容回归。
- `sayall-private-platform/packages/macos-combination-actions` 独立维护免费的组合动作页面、宏库、执行器、输入框学习和本地存储；旧资格客户端仅为兼容保留，不再控制组合动作访问。
- 付费键位方案位于独立的 `packages/macos-button-profiles` Package，不属于组合动作，也不会因组合动作默认集成而自动进入 App。
- 本功能不开放市场、社区上传或任意脚本。

## 关键设计

- 官方构建通过 `SAYALL_COMBINATION_ACTIONS_PATH` 加载 `SayAllMacroRemoteMic`；公开源码 checkout 缺少该路径时仍可完成兼容构建。
- 只有显式设置 `SAYALL_BUTTON_PROFILES_PACKAGE_PATH` 时才加载付费 `SayAllButtonProfiles`，且该变量要求同时存在免费组合动作路径。
- 组合动作运行时不检查邀请码资格；免费 Package 存在时入口和本机执行直接可用。
- 组合动作启动不会显示兑换入口，不会因为打开页面创建设备身份或请求资格服务。
- 未注入组合动作 Package 时使用安全 no-op 适配器，保持 SwiftPM 构建和稳定功能不变。
...
- 公开仓库不包含宏定义、输入框学习数据、组合动作页面或付费键位方案实现。
...
代码和本地 App 打包接入完成。2026-09-13 已将免费组合动作与付费键位方案拆为两个目录、两个 Swift Package；官方构建默认要求免费组合动作，付费键位方案只在显式设置变量时集成。历史邀请码流程的时间解析、本地化和资源加载问题已经修复；`1.8.22 (114)` 暴露的组合动作页面资源崩溃也已通过统一资源入口和双重构建门禁修正。2026-08-16 新增按 Identifier 运行本机快捷指令、受限“打开网址”以及复用已有组合动作；嵌套执行按最新已保存版本解析，并防止循环和过深嵌套。2026-08-18 起组合动作不再要求邀请码，Package 存在时直接显示入口并允许本机执行；旧资格客户端和本机数据只为兼容保留。2026-08-19 宿主页面补充输入框学习与回眸 MCP 的职责边界说明，不改变组合动作执行或 MCP 配置。真实快捷指令、真实默认浏览器、首次隐私授权、跨 Mac 导入、嵌套遥控器链路、Intel、真实遥控器、第三方 App 输入框及 `800 × 650` 页面仍待人工验收。
