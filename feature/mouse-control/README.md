...
- 新增内部动作"切换鼠标模式"（不改变其他配置，绑定方式与其他动作相同）：
  ...
  - 模式内方向键支持固定双击手势（不可配置）：双击上 = Page Up、双击下 = Page Down、双击左 = Command-[（浏览器返回上一页）、双击右 = Command-W（关闭当前窗口）。判定对齐 App 现有手势识别器的 300ms 双击窗口：第一次按下到松开 < 300ms 记为轻点，第二次按下距第一次松开 ≤ 300ms 触发；触发时光标弹回第一次按下前的位置，第二次按下期间该方向不移动，松开后恢复正常；第一次为长按（≥300ms）不构成轻点。
...
- `Sources/RemoteMic/MouseModeController.swift`：新状态机（`idle`/`active`），60Hz 移动引擎（二次缓入：160px/s 起步，约 1.2 秒达到 1400px/s），坐标钳制到屏幕边界，方向 down/up 驱动；方向键双击检测复用 `HIDRemoteTiming.doubleClickMilliseconds`（300ms）作为轻点时长与双击窗口，命中后经可注入 `keyPoster` 走 `KeyboardInjector.postKey` 链路注入固定按键，并把光标弹回第一次按下前位置；OK 三手势状态机复用同一 300ms 窗口做单击待决左键 / 双击右键判定，复用 `HIDRemoteTiming.longPressMilliseconds`（550ms）做长按发送判定。
...
- 单元测试：`Tests/RemoteMicTests/MouseModeControllerTests.swift` 共 30 个用例（状态机进出、辅助功能权限缺失拒绝、方向 down/up 驱动移动、二次缓入加速锚点（t=0→160、t=0.6s→470、t=1.2s→1400、饱和 1400）、斜向归一化、屏幕边界钳制（钳到 maxX-1）、OK 三手势（单击待决 300ms 后左键/双击右键且不误触左键/第二击长按发送/窗口外第二击为再次单击/长按优先/deactivate 清待决）、菜单与返回键穿透且只有切换键退出、非管理键穿透、停用后取消 tick、点击注入与权限 guard、`toggleMouseMode` 内部动作边界、displayName/category/Codable 往返、tick delta 0.05s 上限、光标位置缺失时不移动不点击、HID 监视器 flush 清除幻影长按/双击/连发/非连发锁存、四个方向双击映射与弹回、300ms 窗口内/外边界、第一次长按不构成轻点、单击超时无动作、模式外不响应），已通过。
- 回归：ShortcutCaptureMonitorTests、VoiceFnTapSessionControllerTests、RemoteButtonGestureRecognizerTests、LocalizationTests（含中英文键一致性）以及 Self Test（42 项）均通过；合并运行共 57 项 swift-testing 用例全过。
...
- 2026-08-22 双击手势：方向键双击检测在 MouseModeController 内实现，仅模式内生效，模式外零影响；采用"立即移动 + 弹回"冲突处理，第一次按下前的光标位置在轻点成立时留存，双击命中时 postMove 弹回并注入 Page Up/Page Down/Command-[/Command-W。
