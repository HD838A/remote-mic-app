...
自动化 236 项、22 个 Suite 全部通过，覆盖按 App/日期写入、排序、`0600/0700` 权限、可恢复删除、默认关闭及持久化、连续文字提取、快速发送、连续语音、恢复非空原稿不误存、敏感目标拒绝、无候选焦点变化取消和关闭开关取消捕获；本轮静态回归同时覆盖顶部应用栏、最近使用排序、单行横向浏览、页面内自适应网格和日期折叠结构。`1.8.5 (68)` Apple Silicon 公开版 Release App 已通过资源、架构、最低系统版本、RPath 和深度签名校验，并成功启动。当前桌面使用真实的 2 个 App、1 个日期、12 条记录验证了展开/收起、单 App 筛选、日期折叠恢复和默认总时间线；10 App 横向滚动、跨日期默认折叠和浅色/深色切换仍需在对应真实数据与系统外观下人工验收。人工测试步骤见 [`Testing/LocalTranscriptHistory.md`](../../Testing/LocalTranscriptHistory.md)。
