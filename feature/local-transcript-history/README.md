...
本功能只接入 Mac 已有的公共语音开始/结束生命周期，不修改蓝牙、ATVV、PCM、Fn 注入、iOS、Watch 或网页协议。它不提供录音回放、云同步、全文搜索、智能后处理、跨设备同步或服务端备份。可授权的只读 MCP 由独立功能档案 [`本地 Agent 访问集成`](../sayall-mcp-integration/) 记录，仍不改变本功能的写入和删除链路。

## 关键设计与开发过程

语音开始时通过 Accessibility 保存安全可编辑目标的最小快照；组合动作刚切换 App、输入焦点尚未稳定时，会在 1.25 秒内短时重试，不阻塞音频。语音结束后最多等待 8 秒，只接受原选区处的一段连续新增或替换，并在文字稳定约 900ms 后保存。若已观察到合法候选，用户快速发送、切走焦点或立即开始下一段语音时会先结算该候选；下一段开始前还会进行一次最终安全快照，避免文字刚出现而轮询尚未来得及执行。尚未观察到候选、目标 App 改变或周围文字发生无关变化时仍放弃本次捕获。密码、Token、搜索框、地址栏和设置类输入区域不会记录。
