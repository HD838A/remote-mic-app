...
公开版本在没有私有组件时不显示相关入口。包含私有组件的构建默认同样隐藏 AI 整理邀请码入口；用户在“关于”页连续点击当前版本号 5 次后，可以原地输入邀请码，不需要通过命令行重新启动 App。
...
- “关于”页只负责五击计数；第五次点击调用私有组件的展示接口，不创建匿名设备 ID、不验证邀请码，也不触发资格网络请求。资格请求只能由用户输入邀请码并主动验证后发起。
- 未授权和新授权状态下 AI 整理本地开关均默认关闭，不因入口显示或资格验证自动开启。
...
当前状态：已完成拆分、构建边界和 v1.8.15 邀请入口集成，等待预览版稳定基线与发布门禁验证。
