...
本次只调整“关于”页和更新信息展示，不改变 Sparkle 的下载、授权、安装、重启、自动检查周期或稳定 feed 地址。不会自动安装更新，也不会让预发布版本进入默认正式更新通道。
...
- 正式/预发布开关变化后清除旧页面状态和预发布 URL，重置 Sparkle 更新周期，再检查新通道。
- 预发布 Release API 不可用时静默回退稳定 feed，不显示候选源错误。
...
更新检查只访问既有 GitHub Release API、appcast、版本 ZIP 和更新说明 URL，不上传本地设置、设备信息或使用数据。稳定 feed 保持不变；`CFBundleVersion` 仍由 Sparkle 用于跨版本比较。更新说明获取失败不会阻止 Sparkle 检查或安装。

## 自动化验证与当前限制

单元测试覆盖预发布 appcast 选择、固定版本说明 URL、说明解析、语言重载和关于页结构。Sparkle CLI 探测覆盖多个旧版本到正式/预发布版本的版本比较。真实升级、签名、公证、最终安装包启动和最小窗口人工验收完成后，状态才改为“已完成”。
