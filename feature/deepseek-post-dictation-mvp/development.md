...
- AI 整理与 Early Access 定向回归：24 项测试、10 个 Suite 通过；
- `swift test --jobs 4`：232 项测试、30 个 Suite 通过；
- 私有硬件模拟回归：16 项测试、1 个 Suite 通过，覆盖 RC003 无前置 `MIC_OPEN` 的普通语音路径；
...
自动化测试不能证明真实豆包输入法、Accessibility 或第三方输入控件已经兼容。当前 Mac 在本轮验收时处于锁屏状态，Computer Use 无法执行设置页逐页点击；当前状态因此是“等待人工验收”，不能表述为已完成真实输入流程验收。
