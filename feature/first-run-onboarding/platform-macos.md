...
| 转写路线 | 本机粤语/英语 Whisper 模型优先；Typeless、其他支持语音输入的工具保留为可选旧路线。已选豆包或微信的旧配置可继续显示，但不推荐给新用户 |
| Onboarding 语音键 | 豆包、微信和其他工具使用 Fn/地球键长按；Typeless 使用 Fn 点按开始/结束 |
| 音频路线 | 本机模型直接接收遥控器 PCM；只有第三方路线需要受支持的虚拟音频设备及匹配的麦克风设置 |
| 输入目标 | 原生 AppKit 文本编辑器必须成为当前 key window 的 first responder |
| 完成证据 | 当前来源连接、普通按键、语音开始、真实样本、语音结束、所选路线的文字写入和三个不同普通按键；第三方路线还须证明音频投递 |
...
- 第三方语音工具的 Onboarding 统一使用 Fn/地球键，不在首次流程中提供左/右 Command 选择；本机转写直接由遥控器语音键启动，不向任何输入法发送快捷键。
- 选择第三方工具时，旧设置为左/右 Command 应切回 Fn，并显著提示用户发生了什么变化；本机转写保留旧按键设置但不使用它唤起外部工具。
...
- 本机转写不要求虚拟音频设备，必须等模型加载成功；第三方路线只接受产品明确支持的虚拟音频设备。
- 本机路线要将实体遥控器语音键的硬件 Fn 事件中和，直接收集 PCM，并且不合成 Fn/Command 语音快捷键；若无法可靠中和硬件 Fn，须阻止该设备的本机语音会话并显示失败。模型加载失败时允许用户在 Onboarding 和连接页重试。
