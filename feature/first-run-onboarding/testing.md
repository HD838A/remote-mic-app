...
2026-09-06 实体遥控器普通按键指引候选复验：Onboarding 49 项、Localization 6 项、完整 Swift 459 项/39 suites、项目自检 44/44 与 Apple Silicon Release 构建通过；实体遥控器、iPhone、网页版三条分支共 58 张浅色/深色生产视图截图，以及实体遥控器误按语音键状态浅色/深色截图均逐张检查通过。自动化确认语音键不会满足普通控制键门槛，诊断能够区分语音键与普通控制键；真实 RC003 ATVV/HID 事件和用户完整现场流程仍需候选包真机验收。
...
- 遥控器页在任何按键到达前明确要求短按圆盘中间确定键或方向键并排除麦克风/语音键；误按语音键后立即显示纠正状态，仍不能继续；随后按普通键才恢复并通过；
...
- 遥控器页误按语音键后的诊断包含语音键触发次数、普通控制键观察次数和最后输入类型，能够与完全没有输入事件区分。
