...
3. Typeless 分支在权限通过后启用现有 Fn 点按模式；豆包和其他工具关闭该模式。音频步骤复用生产设备列表与配置链路，但 Onboarding 只展示并接受 `DoubaoAudioDevicePolicy` 识别到的 MiRemoteV 2ch；主设置页仍保留普通设备选择。
...
23. iPhone/网页语音输出采用按需生命周期：手机语音开始时配置，停止后释放。音频页和完成页要求 MiRemoteV 2ch 已选择且仍存在，真实会话、PCM、停止和文字继续在语音页验证；实体遥控器继续要求生产输出持续 Ready。三种控制方式统一检查蓝牙、输入监控和辅助功能；权限页不伪造无法可靠查询的本地网络授权状态，而是在连接页用 Nearby/WSS 真实会话作为门禁。
24. 语音测试的文字状态不再等同于输入框非空。每次所选来源开始语音时清空旧文字；聚焦输入框内由本地键盘事件产生的文字会标记为手动输入并阻塞继续，诊断只记录布尔状态，不记录文字内容。
...
1. 单元测试覆盖步骤顺序、前后导航、能力门禁、延迟运行时、退出续接、完成版本、重新运行、MiRemoteV 2ch 唯一门禁和手动键盘文字拒绝，防止 UI 调整改变流程语义。
...
4. 系统权限弹窗、真实 RC003、iPhone Nearby、手机 Safari 网页版、MiRemoteV 2ch、豆包、Typeless 和其他工具仍属于真实环境验收；自动化只能证明其他设备被拒绝，不能替代真实文字写入来源判断。
...
- `swift test --filter OnboardingFlowTests`：2026-08-18 最新 23 项通过，覆盖两层控制方式选择、连接恢复、MiRemoteV 2ch 唯一门禁、手动键盘文字阻塞、输入框重新聚焦、最终运行时重验和迁移边界。
