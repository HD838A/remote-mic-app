...
- `Sources/RemoteMic/OnboardingView.swift`：双栏页面、三种控制方式、权限操作、连接与按键状态、真实语音文字测试和固定底部导航。
...
20. 控制方式页按“实体遥控器 → 推荐 iPhone App → 没有 iPhone 时使用网页版”平铺三张卡。中途升级且已经越过该页的旧流程默认保持实体遥控器，控制方式页退出重开时保持未选择。
...
| 步骤 | 状态来源 | 恢复方式 | 自动化边界 |
| --- | --- | --- | --- |
| 控制方式 | 本机持久化选择 | 返回选择页改选；重新运行时重新选择 | 不能替代对应真实设备 |
...
- `swift test`：2026-08-17 三种控制方式接入后 232 项、20 个 suite 全部通过；覆盖分支权限、持久化、按键和语音来源接线、9 页截图契约。最终提交前重新执行并以最新结果为准。
...
- 实现截图：已完成。无需屏幕解锁，直接实例化生产 `OnboardingView`，通过离屏 AppKit 窗口缓存为实体遥控器、iPhone 和网页版分别生成浅色、深色各 9 张，共 54 张真实实现截图；逐页确认控制方式页、连接分支、底部导航和实时检查区域无裁切，未使用设计稿替代实现证据。
- 2026-08-17 布局回归复验：豆包浅色/深色四个指引状态、微信浅色/深色语音工具页，以及三种控制方式浅色/深色全部 9 页均已生成并逐张检查；工具卡对齐，右栏被完整利用，全流程无页面内部滚动、裁切或黑白分栏。
