...
31. 实体遥控器连接页同时观察生产 HID 普通按键和当前来源的语音会话开始事件。HID 普通按键仍是唯一通过门槛；如果用户误按麦克风/语音键，页面立即显示橙色纠正卡并指出应按圆盘中间确定键或方向键。`FirstUseDiagnostics` 保存两类输入的脱敏计数和最后输入类型，普通按键到达后清除纠正状态；不修改 BLE、HID、ATVV 或音频实时链路。
