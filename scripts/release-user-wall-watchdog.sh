...
if [[ "$#" -ne 5 ]]; then
  print -u2 "usage: $0 preview|stable <request-started-epoch> <candidate-branch-or-tag> <completion-file> <run-id-file>"
  exit 2
fi
case "$MODE" in
  preview)
...
if (( initial_now - REQUEST_STARTED_AT < 0 )); then
...
while true; do
  ...
  if (( elapsed >= BUDGET_SECONDS )); then
    cancel_registered_runs
    print -u2 "RELEASE USER-WALL SLO EXCEEDED mode=$MODE target=$TARGET elapsed=${elapsed}s budget=${BUDGET_SECONDS}s"
