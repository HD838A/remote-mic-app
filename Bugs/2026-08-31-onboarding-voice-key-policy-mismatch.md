...
- 流程自动化：`swift test --filter OnboardingFlowTests` 通过（38 项）；覆盖 Fn-only 门禁、旧 Command 迁移、重跑不恢复旧模式、三种控制方式门禁、返回导航和完成后设置保留。
...
- 全量 Swift 测试：修改前已通过 427 项/37 个 suite；本次诊断架构字段修正后重新通过受影响的 Onboarding 与本地化测试，完整套件将在提交前再次执行。
