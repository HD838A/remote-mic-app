...
- 新增硬件模拟回归，覆盖“多次 App 连发 + 单次真实松开”和“按下期间断连且没有松开报告”两种场景。

本次修复遵循：事件回放复现 → 核对现有日志能力与事件路径 → 用最小实验确认状态泄漏 → 实施最小修复 → 重跑原始复现和稳定基线。

## 验证边界

- 已完成：修复前失败复现；独立硬件模拟跨项目 13 项通过，其中新用例直接驱动生产 monitor、调度器和 suppressor；既有 7 个连发按键、36 个手势、12 个原始按键和异常/断连基线均通过。
- 已完成：`swift test --filter RemoteButtonsTests` 82 项通过，`scripts/test.sh` 42 项通过，Release App 构建成功。
- 环境边界：完整 `swift test` 已完成构建并开始执行，但被仓库基线中现有的 Keychain 测试长时间阻塞后人工中止；该现象不在本次方向键修改范围内，不能表述为完整套件通过。
