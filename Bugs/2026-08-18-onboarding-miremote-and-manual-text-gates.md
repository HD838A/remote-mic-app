...
- 修改后：Onboarding 定向测试 23 项通过，覆盖 MiRemote/BlackHole 允许、普通输出拒绝、受支持设备缺失、手动键盘输入阻塞、真实文字状态及对应失败码。
- 完整 Swift 测试 235 项、项目自检 42 项和 Debug 构建通过。
- 中英文字符串与 diff 格式检查通过；实体遥控器路径音频页、语音页的浅色和深色生产截图已逐张检查，无裁切、内部滚动或黑白分栏。

## 验证边界

自动化不能证明豆包、微信输入法、Typeless 或其他语音工具在真实 Mac 上写入文字时不会被系统标记成键盘事件。真实 `MiRemoteV 2ch`、`BlackHole 2ch`、实体遥控器、iPhone、网页版及第三方语音工具仍需按测试手册验收。
