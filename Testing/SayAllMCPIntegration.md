...
预期结果：已安装客户端显示可连接，未安装客户端不可误连接；每个客户端获得不同授权；成功后提示重启；Codex 只维护 SayAll 标记区块，Cursor/OpenCode 只合并或移除 `sayall_history`，Claude Code 使用官方用户级 CLI；修改前创建私有备份；客户端或配置预检失败时不创建授权，安装中途失败时丢弃临时授权；同一客户端最多一份有效授权；授权列表只显示有效授权；错误文案明确指出客户端和 CLI、配置格式、同名冲突、路径、权限或安装命令原因。
...
自动化覆盖 v1 状态、默认关闭、令牌哈希、权限、未来 Schema 拒绝、错误凭据、有效授权唯一性、撤销、历史解析、时间/App 筛选、排序、分页、内部字段排除、配置生成、四客户端预检、安全合并与移除、冲突拒绝、失败临时授权丢弃、撤销记录隐藏、唯一 `serve` 入口和打包接线。完整 `swift test` 共 262 项、27 个 Suite，通过；其中快速连接 13 项。临时数据真实 stdio 已验证初始化、`tools/list`、两个工具、默认关闭拒绝、stdout/stderr 分离和 `nextCursor: null`；四份公开 Schema 与 Helper 完全一致。
