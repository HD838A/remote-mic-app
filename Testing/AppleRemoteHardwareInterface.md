...
预期结果：指针移动稳定且相较旧包明显更快、轻触只点击一次、外圈可以连续滚动且步幅足够；外圈向中心的径向轨迹锁定为鼠标移动，累计圆周轨迹达到阈值后锁定为滚动，同一次接触不会在两种模式之间切换。移动和滚动指示器固定优先显示在鼠标右下方，贴近但不覆盖鼠标；两种标识大小一致，并按相同速率随速度放大或缩小。鼠标停在公开 Accessibility API 报告自身或最多四层父元素支持 `AXPress` 的目标上时，指示器延长显示，延长期内一次 OK 只触发一次该元素；滚动、触摸点击、超时、断连或 App 停止都会取消待点击目标。触摸结束后不漂移。

失败判定：完全没有触摸回调、指示器未出现或覆盖鼠标、两种标识大小/缩放速率不同、可点击元素上的延长窗口未出现、OK 重复执行或执行旧目标、App 退出、点击重复、径向滑动触发滚动、圆周滑动移动鼠标、同一次接触反复切换模式、滚动方向相反、快速滚动失控、移动或滚动仍明显迟缓，或实体按压附带鼠标动作。
