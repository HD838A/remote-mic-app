...
5. 观察鼠标右下角的指示器；移动与滚动标识应使用同一基础大小和同一速度缩放曲线，距离鼠标保护边界约 4 pt。把鼠标移到屏幕四角后重复移动与滚动。
6. 把鼠标停在 Safari 链接、Finder 按钮、标准 SwiftUI/AppKit 按钮和 Electron 按钮上；停止移动后等待约 0.3 秒，再在指示器延长显示的 1.2 秒内按中心 OK。

预期结果：指针移动稳定且相较旧包明显更快、轻触只点击一次、外圈可以连续滚动且步幅足够；低速滚动可精确控制，正常到快速旋转时页面速度单调增加且不突跳。外圈向中心的径向轨迹锁定为鼠标移动，累计圆周轨迹达到阈值后锁定为滚动，同一次接触不会在两种模式之间切换。移动和滚动指示器固定优先显示在鼠标右下方，贴近但不覆盖鼠标；两种标识大小一致，并按相同速率随速度放大或缩小；滚动弧线箭头清晰可辨。鼠标停在公开 Accessibility API 报告自身或最多四层父元素支持 `AXPress` 的目标上时，指示器延长显示，延长期内一次 OK 只触发一次该元素；滚动、触摸点击、超时、断连或 App 停止都会取消待点击目标。触摸结束后不漂移。
...
7. 顺时针和逆时针转动外圈，确认光环短暂变为对应方向的旋转弧线；分别切换系统自然滚动开关，确认弧线箭头仍跟随手指物理旋转方向，而不是页面上下移动方向。
8. 在支持 `AXPress` 的标准按钮或链接上停止移动，确认光环延长约 2.5 秒；期间分别测试实体 OK 和中心区域短促静止轻触，两种方式都只激活目标一次。中心轻触不得执行 OK 的键位映射，也不得额外产生第二次鼠标点击。
9. 在确认窗口内移动光标、开始滚动、等待超时、开始语音、断开遥控器，并分别再次触摸或按 OK，确认旧目标已取消且普通输入恢复。

预期结果：指针稳定，无明显静止漂移；点击只触发一次；实体方向环或中心按压不附带移动或点击；径向轨迹锁定鼠标、圆周轨迹锁定滚动且抖动不切换；原生光标保持可见，主题色光环围绕光标，滚动弧线方向与手指一致且不受自然滚动设置影响；悬停确认窗口只消费一组实体 OK began/ended 或一次中心轻触，二者都不会产生双击。滚动低速可精确控制、快速不突跳。触摸结束和断连不会留下输入状态。

失败判定：Hardened Runtime 下进程退出、触摸无回调、光环仍作为独立图标显示、原生光标被隐藏、光环明显偏离光标主体、弧线方向与手指相反或随自然滚动开关反转、中心轻触执行键位映射或重复点击、目标超时后仍被误触、径向/圆周误判、同一次接触切换模式、实体按压附带鼠标动作、滚动跨窗口或快速滚动失控。
...
- 已由自动化覆盖：精确 VID/PID/usage page 筛选、实体键 usage 映射、媒体/静音/电源通用映射、原生事件抑制候选、原始 control ID 活动态、电源键左列、右键列、左右列数量平衡、与 RC003 对齐的 `85.5 pt` Siri 卡片间距、任意非零 HID 值保持 pressed、设备实例隔离、即时 down/up、重复 down、无匹配 up、断连取消、配置切换后的迟到 up、确定性取消顺序、单调 sequence、Apple 语音 owner 隔离、设备档案不污染小米蓝牙档案、A2854 中心移动/轻触/外圈径向与圆周分类/模式锁/阈值抖动/物理旋转方向/实体按压抑制/取消复位、原生光标环绕布局、中心轻触确认单次消费和回退、防双击、目标超时与光标移走取消、语音活动与尾音排空期间触摸抑制、抑制接触必须抬手后恢复、按住连发策略，以及 PacketLogger 文本解析、pklg ACL 分片重组、IPC 分片解码和 PCM 编解码。
