...
6. 检查移动/滚动指示器位于鼠标右下、距离保护边界约 4 pt 且不重叠；两种标识使用相同基础大小和速度缩放，并覆盖屏幕四角的自动换侧。
7. 在支持 `AXPress` 的标准按钮或链接上停止移动，确认标识延长约 1.2 秒；期间按一次 OK 只激活一次，超时后 OK 恢复普通映射。

预期结果：指针稳定，无明显静止漂移；点击只触发一次；实体方向环或中心按压不附带移动或点击；径向轨迹锁定鼠标、圆周轨迹锁定滚动且抖动不切换；指示器可见但不覆盖系统鼠标，移动/滚动尺寸和速度曲线一致；悬停点击窗口只消费一组 OK began/ended。滚动方向一致且低速可精确控制、快速不突跳。触摸结束和断连不会留下输入状态。

失败判定：Hardened Runtime 下进程退出、触摸无回调、指示器缺失或遮挡鼠标、点击重复、径向/圆周误判、同一次接触切换模式、实体按压附带鼠标动作、滚动反向或跨窗口、快速滚动失控。
...
- 已由自动化覆盖：精确 VID/PID/usage page 筛选、实体键 usage 映射、媒体/静音/电源通用映射、原生事件抑制候选、原始 control ID 活动态、电源键左列、右键列、左右列数量平衡、与 RC003 对齐的 `85.5 pt` Siri 卡片间距、任意非零 HID 值保持 pressed、设备实例隔离、即时 down/up、重复 down、无匹配 up、断连取消、配置切换后的迟到 up、确定性取消顺序、单调 sequence、Apple 语音 owner 隔离、设备档案不污染小米蓝牙档案、A2854 中心移动/轻触/外圈径向与圆周分类/模式锁/阈值抖动/方向/实体按压抑制/取消复位、指示器右下布局与屏幕边缘换侧、语音活动与尾音排空期间触摸抑制、抑制接触必须抬手后恢复、按住连发策略，以及 PacketLogger 文本解析、pklg ACL 分片重组、IPC 分片解码和 PCM 编解码。
