# 苹果遥控器通用硬件接口与 Type-C/Lightning 实机测试手册

## 适用范围

- 日期：2026-09-12。
- 目标硬件：苹果遥控器 Lightning（硬件代号 A2540）与苹果遥控器 Type-C（硬件代号 A2854）。面向用户的界面只显示连接器名称，不显示硬件代号；其他代际不在本手册兼容范围内。
- 当前状态：已实现 A2540/A2854 候选适配器、通用生命周期接线、设备档案、Siri 键即时会话、候选触摸输出，以及由安装包管理的系统 PacketLogger/HCI → Opus → PCM → `MiRemoteV 2ch` 音频桥接。A2540 当前仍为 `awaitingRealHardware`，完成本手册实机矩阵前不得标记为正式兼容。
...
1. 确认遥控器为苹果遥控器 Lightning 或苹果遥控器 Type-C，并在测试记录中注明连接器版本。
...
预期结果：连接状态显示苹果遥控器已连接，设备选择区分别显示“苹果遥控器 Type-C”或“苹果遥控器 Lightning”，不出现 A2854/A2540。两款遥控器使用同一张产品图片、比例和锚点。失败判定：授权并重启后仍只显示正在查找、名称含硬件代号、两版本布局不一致，或者按键会操作系统但 App 没有任何响应。
...
| App 切换器完整生命周期 | 已对齐 RC001/RC003：Command 持续按下、TV 继续 Tab、左右 Cmd+导航、中心确认、返回取消、超时/断连清理 | 自动化已覆盖合成键生命周期；仍需 A2854 真机确认系统 App 切换器视觉与前台切换稳定 |
...
| 电池/充电状态 | 候选公开读取；系统未提供电量且充电状态也未知时隐藏整个电池区域 | 有真实电量或明确充电/外接供电时才显示；不得显示空电池图标或“—”冒充状态 |
| 物理键能力差异 | 播放/暂停、静音和电源均进入独立 A2854 档案映射；A2854 没有 RC 的 Home/Menu 同构按键 | 自定义映射启用时原生系统副作用被抑制；UI 不展示不存在的 Home/Menu |

快速连按和电池状态仍是后续对齐项；App 切换器生命周期已在本次修复中补齐，但真机视觉验收仍未完成。

### 页面与 App 切换器回归（本次修复）

1. 在连接页分别选择 RC003 与 A2854：两者缩略图必须使用同一外框尺寸，设备切换不能改变右侧面板几何。
2. 在按键页向下滚动到编辑器或页脚，再从 A2854 切换到 RC003（以及反向切换）：新设备页面必须从顶部开始，不得停留在旧页面的下半页。
3. 将 A2854 的 TV 键设为 `Command-Tab`，按 TV 后保持 Command-Tab 界面打开；按左右键只移动选择，不得直接触发普通方向动作；按中心确认，按返回取消。
4. 在未确认或断连场景等待超时：Command 必须释放，日志必须出现 `APPLE REMOTE APP SWITCHER phase=ended`，不得留下卡住的修饰键。

失败判定：连接页仍显示另一型号图片、切换设备后保留旧滚动位置、左右键导致普通动作或选中错误 App、松开/断连后 Command 仍保持按下，或 TV 释放额外产生一次普通动作。
...
3. 如果语音键模式选择左 Command 或右 Command，先在目标输入法中配置同一侧 Command；分别验证一次左侧和右侧，目标输入法必须能区分物理侧别。日志中的 `result=event_submitted target_response=unknown` 只表示系统事件已提交，不代表输入法已经响应。
4. 连续做 10 次很快的“按下后立即松开”。
5. 再做 3 次正常按住 2 至 5 秒后松开，说一小段可识别的话。
6. 最后按住 Siri 键时关闭遥控器连接或退出 App，再重新连接并做一次正常按住。
7. 每次语音按住期间分别移动手指、轻触和转动触摸盘；松键后继续保持当前接触，再抬手并开始一次新触摸。

预期结果：按下立即开始会话，松开后等待尾音自然排空再完成；helper 报告 `capturing`，日志出现遥控器麦克风首批 PCM，并且 `MiRemoteV 2ch` 电平随遥控器说话变化；豆包最终收到遥控器声音。若 HCI/PacketLogger 尚未就绪，输入法仍保持唤醒，日志出现 `audio_capture_not_ready` 和有限重试，随后在就绪后开始捕获；不得在同一按键按住期间立即合成 Fn up。极速短按不丢失，断连或退出后 Fn/语音状态不会卡住，重连后的第一次操作即可成功。语音活动及尾音排空阶段不产生触摸移动、滚动或点击；语音期间已经开始的接触不会在恢复瞬间补发点击，抬手后的下一次新触摸正常工作。
...
2. 测试慢速、正常和快速一圈，观察滚动方向、步幅和是否突跳；在相同方向、相近半径和相近总旋转角度下，快速旋转的页面滚动速度必须明显高于慢速旋转。
3. 按住方向环或中心实体键时移动手指，确认不会同时移动鼠标或点击。
4. 从外圈向中心做 10 次直线滑动，再沿外圈做 10 次圆周滑动；每次滑动期间故意加入轻微抖动。
5. 观察围绕原生鼠标的主题色光环；移动与滚动反馈应使用同一基础大小和同一速度缩放曲线。滚动时右侧缺口显示页面向上/向下箭头，数量随速度在 1～3 个之间变化。把鼠标移到屏幕四角后重复移动与滚动。
6. 按 TV 键打开 Cmd+Tab App 切换器，不按方向键，分别缓慢和快速顺时针转圈，再逆时针转圈；最后分别用确定键确认、返回键取消，并等待超时自动退出。
6. 把鼠标停在 Safari 链接、Finder 按钮、标准 SwiftUI/AppKit 按钮和 Electron 按钮上；停止移动后等待约 0.3 秒，再在指示器延长显示的 1.2 秒内按中心 OK。

7. 在 Cmd+Tab 界面保持触摸面静止后轻点一次，不按实体确定键；再重复一次旋转后轻点确认，并分别测试返回、超时和断连。

预期结果：指针移动稳定且相较旧包明显更快、轻触只点击一次、外圈可以连续滚动且步幅足够；低速滚动可精确控制，正常到快速旋转时页面速度单调增加且不突跳。外圈向中心的径向轨迹锁定为鼠标移动，累计圆周轨迹达到阈值后锁定为滚动，同一次接触不会在两种模式之间切换。移动和滚动反馈围绕原生鼠标且不覆盖点击热点；两种反馈大小一致，并按相同速率随速度放大或缩小；滚动缺口箭头清晰表示页面上下方向，数量随速度从 1 到 3 单调增加。鼠标停在公开 Accessibility API 报告自身或最多四层父元素支持 `AXPress` 的目标上时，指示器延长显示，延长期内一次 OK 只触发一次该元素；滚动、触摸点击、超时、断连或 App 停止都会取消待点击目标。Cmd+Tab 显示期间不移动鼠标、不触发普通点击或垂直滚动；顺时针转圈稳定选择右侧/下一项，逆时针选择左侧/上一项；在圆周模式静止轻点应确认当前 App，不发送鼠标点击；退出切换器后下一次触摸恢复普通指针与滚动。触摸结束后不漂移。每轮日志必须具有唯一 `operation_id`，并在确认后的 0/150/500/1000 ms 记录 `phase=visibility_probe`；最终必须且只能出现一个 `phase=terminal`，其中 `visible_target_confirmed` 才能证明目标 App 同时为前台且存在屏幕上的普通窗口。`success=true`、`command_release=true` 或前台 Bundle ID 变化均不得单独判定为用户可见成功。

AppSwitcher 连续使用专项：连续执行至少 10 轮 TV → 圆周顺/逆时针 → 触摸轻点确认，再各执行 5 轮 TV → 实体左右 → 实体确定确认。微信需分别处于未运行、后台有窗口、已隐藏或后台无普通窗口三种可建立的公开状态。每轮检查 `operation_id` 不串轮，聚合计数与实际操作一致，`is_active_after=false`，Command 释放成功，并依据终态判断用户是否应看到目标窗口。任何一轮只有事件提交、没有唯一终态，或终态为 `frontmost_changed_no_visible_window` / `frontmost_unchanged` / `diagnostic_unknown`，均判为失败并保留该 ID 的完整日志。

失败判定：完全没有触摸回调、指示器未出现或覆盖鼠标、两种标识大小/缩放速率不同、滚动箭头不可辨识、可点击元素上的延长窗口未出现、OK 重复执行或执行旧目标、App 退出、点击重复、径向滑动触发滚动、圆周滑动移动鼠标、同一次接触反复切换模式、滚动方向相反、快速旋转没有比慢速更快、滚动速度不单调或快速滚动失控、移动或滚动仍明显迟缓、Cmd+Tab 期间触摸导致鼠标移动/轻点确认/页面滚动、转圈方向错误、退出后仍停留在圆周导航模式，或实体按压附带鼠标动作。

### 5.1 Magic Mouse 共存回归

1. 同时连接一只 Magic Mouse 和一只 A2540/A2854，记录启动 App 前的移动与滚动体感。
2. 启动 App，分别操作 Magic Mouse 与 Siri Remote 触摸面；退出并再次启动 App，重复三次。
3. 检查日志只记录稳定遥控器型号，不记录普通鼠标身份。

预期结果：无线麦运行、退出与重启都不得改变 Magic Mouse 的指针或滚动速度；Siri Remote 触摸仍正常。失败判定：App 运行时普通鼠标变慢、卡顿、滚动改变，或触摸桥绑定 Product ID `0x0269`。
...
带 Siri Remote 语音能力的 Release `.app` 必须使用 Team ID `L3QHLDRPAY` 的 Developer ID Application
签名。`scripts/build-app.sh` 默认拒绝私有 Siri Remote 包的 ad-hoc 构建，`scripts/verify-app.sh` 也会拒绝
ad-hoc 或其他 Team ID 的 Siri Remote App；这是因为已安装的特权 HCI Helper 会按签名身份拒绝该客户端。
ad-hoc 包可以用于不启动 HCI 语音链路的 UI/编译检查，但不能作为语音真机测试包。
...
| 最大加速度倍率 | `2.6` |
| 初始滚动反向 | `false` |
| 滚动箭头视觉方向 | 默认与页面右侧滚动指示器一致；可设置为相反，不改变实际滚动 |
| 中心轻触确认最大归一化半径 | `0.28` |
| 可点击目标确认窗口 | `2 秒` |
...
| 设备组合 | 仅 Lightning、仅 Type-C、苹果遥控器 + Magic Mouse、苹果遥控器 + RC001/RC003、两只同版本设备（条件允许时） | 同名控制项按设备实例隔离；普通鼠标性能不变 |
...
| 场景 | 操作 | 通过标准 |
|---|---|---|
| 首次安装 | 安装 `Install SayAll.pkg`，启动 App，首次进入 Apple Remote 音频 | Siri Remote 组件默认不选；用户主动选中后安装；仅出现一次标准管理员认证，不要求 PacketLogger 或 Apple 账号 |
| 已安装升级 | 在 receipt、HCI Helper 或 LaunchDaemon 已存在时打开新 `Install SayAll.pkg` | Siri Remote 可选组件自动勾选，用户仍可手动取消；`installer -showChoicesXML` 的 `choiceIsSelected=1` |
...
1. 分别在系统蓝牙中配对 A2540 与 A2854 并启动候选 App。
2. 检查适配器只接受批准的 Apple Vendor/Product/usage 组合。
3. 连接其他 Apple 蓝牙设备，确认不会被当作遥控器 seize 或路由。

预期结果：A2540/A2854 均显示为候选设备；其他 Apple 设备不进入遥控器事件链路。两型号连接页视觉一致，日志只记录稳定模型分类和进程内设备槽位。
...
5. 回放外圈向中心的径向轨迹、外圈圆周轨迹和阈值附近抖动；确认单次 contact 只锁定一种模式。
6. 检查触摸移动时主题色光环完整围绕原生光标主体；原生箭头、I-Beam、链接手型和窗口缩放光标仍由系统正常显示。光环按速度平滑缩放，不在光标旁边出现第二个独立图标。
7. 顺时针和逆时针转动外圈，确认光环右侧缺口只显示页面滚动指示器的向上或向下箭头；慢速为 1 个、正常为 2 个、快速为 3 个。分别选择“与页面滚动指示器一致”和“相反”，确认只反转箭头，不改变页面实际滚动方向、自然滚动设置或 iPod 转盘方向。
8. 在支持 `AXPress` 的标准按钮或链接上停止移动，确认光环延长约 2 秒；期间分别测试实体 OK 和中心区域短促静止轻触，两种方式都只激活目标一次。中心轻触不得执行 OK 的键位映射，也不得额外产生第二次鼠标点击。
9. 在确认窗口内移动光标、开始滚动、等待超时、开始语音、断开遥控器，并分别再次触摸或按 OK，确认旧目标已取消且普通输入恢复。

预期结果：指针稳定，无明显静止漂移；点击只触发一次；实体方向环或中心按压不附带移动或点击；径向轨迹锁定鼠标、圆周轨迹锁定滚动且抖动不切换；原生光标保持可见，主题色光环围绕光标，滚动缺口箭头清晰表示页面滚动指示器方向，数量随速度从 1 到 3 单调增加；视觉反向设置不影响真实滚动。悬停确认窗口只消费一组实体 OK began/ended 或一次中心轻触，二者都不会产生双击。滚动低速可精确控制、快速不突跳。触摸结束和断连不会留下输入状态。

失败判定：Hardened Runtime 下进程退出、触摸无回调、光环仍作为独立图标显示、原生光标被隐藏、光环明显偏离光标主体、箭头不是页面上/下方向、箭头数量超出 1～3 或速度越快反而减少、视觉反向设置改变真实滚动、中心轻触执行键位映射或重复点击、目标超时后仍被误触、径向/圆周误判、同一次接触切换模式、实体按压附带鼠标动作、滚动跨窗口或快速滚动失控。
...
“苹果遥控器 Type-C”或“苹果遥控器 Lightning”的设备档案显示，RC001/RC003 继续使用原有小米页面；面向用户的名称不得出现 A2854/A2540。
...
4. 确认左右两列第一个配置项顶部对齐，中心纵坐标均为 `45.6 pt`；相邻卡片中心距与 RC003 一致，为 `85.5 pt`。顶部“电源”键卡片位于左列，“语音”固定卡位于右列，“右”键继续位于右列；两列卡片数量差不超过 1，全部卡片无重叠。
...
9. 使用截图 harness 在 `800 × 650` 做窄宽压力检查，并在真实生产窗口最小尺寸 `1020 × 772` 下完成点击验收；两种尺寸都覆盖浅色与深色外观，确认图片、连接线、卡片、语音卡和底部说明无重叠、裁切或中文缩小到 12pt 以下。不得把 `800 × 650` 表述为真实生产窗口尺寸。
...
- 已由自动化覆盖：精确 VID/PID/usage page 筛选、实体键 usage 映射、媒体/静音/电源通用映射、原生事件抑制候选、原始 control ID 活动态、电源键左列、右键列、左右列数量平衡、与 RC003 对齐的 `85.5 pt` Siri 卡片间距、任意非零 HID 值保持 pressed、设备实例隔离、即时 down/up、重复 down、无匹配 up、断连取消、配置切换后的迟到 up、确定性取消顺序、单调 sequence、Apple 语音 owner 隔离、设备档案不污染小米蓝牙档案、A2854 中心移动/轻触/外圈径向与圆周分类/模式锁/阈值抖动/物理旋转方向/实体按压抑制/取消复位、原生光标环绕布局、页面上下箭头映射、视觉方向反转、1～3 个速度分级与持久化、中心轻触确认单次消费和回退、防双击、目标超时与光标移走取消、语音活动与尾音排空期间触摸抑制、抑制接触必须抬手后恢复、按住连发策略，以及 PacketLogger 文本解析、pklg ACL 分片重组、IPC 分片解码和 PCM 编解码。
