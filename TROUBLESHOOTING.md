...
本文适用于 `1.1.1`。请先确认 Mac 为 Apple Silicon，系统版本为 macOS 26 或更高。
...
`1.1.1` 的菜单动作使用 macOS 原生上下文菜单键，不再使用 `Shift-F10`。如果仍有提示音，请在报告中注明实体按键、当前映射、前台应用和 HID 状态是“独占模式”还是“兼容模式”。
...
```bash
shasum -a 256 -c Remote-Mic-1.1.1.dmg.sha256
```
