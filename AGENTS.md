...
- iOS 的“按住开始、松手停止”和“点按开始、再次点按停止”必须共用同一个底层录音会话状态，不得分别维护两套连接或麦克风状态。
- 权限请求、启动中、已录音、停止中、连接断开和 App 失去活动状态都必须能结束同一会话，避免重复开始、假性录音或后台持续收音；系统权限弹窗本身不得误取消用户刚发起的录音请求。
...
- Push 成功只能表述为“已触发 TestFlight 发布流程”，不能表述为构建已经上传、处理完成或测试员已经可以安装。
