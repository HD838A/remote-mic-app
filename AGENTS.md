...
- “左 Control”只是某个用户配置和真机验收用例，不是产品固定行为。语音键触发目标的唯一来源是当前保存的 `voiceTriggerShortcut` 配置。
- 生命周期恢复、HID 映射、失败重试、状态显示和日志判断不得写死左 Control、Fn 或其他具体按键；独立左右修饰键必须保留键位与左右侧信息，普通单键和组合键继续走各自既有路径。
- 修改语音键映射逻辑时，自动化至少同时覆盖一个非 Control 的独立修饰键以及一个普通键或组合键，防止用当前用户配置代替通用产品能力。
