...
- 每个正式开发的新功能都必须在仓库根目录 `feature/` 下建立独立子目录，目录名使用稳定、可读的 kebab-case 功能标识。
- 功能档案至少记录：为什么开发、用户功能介绍、范围与非目标、关键设计和开发过程、涉及文件、隐私或兼容边界、自动化验证、人工测试手册、当前状态和已知限制。
- `feature/README.md` 维护目录规范和功能索引；功能状态或验证结论发生变化时，应同步更新对应子目录，而不是只留在聊天、提交信息或临时计划中。
- 根目录 `TODO.md` 继续保留任务状态，不搬入 `feature/`；功能档案可以引用对应 TODO，但不能代替 TODO 同步规则。
- 详细 research、长期计划、竞品分析和内部判断继续保存在私有 marketing 仓库；公开 `feature/` 只整理实现所需的结论、实际开发记录、公开行为和验证边界，不复制私有研究全文，也不修改私有仓库原文。
