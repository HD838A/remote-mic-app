...
- 默认优先使用公开 API、公开协议、用户可见的辅助功能界面或用户明确选择的文件。确需读取其他 App 的内部状态时，必须在本节登记精确的 App、文件、字段和用途，只允许只读、最小化访问，并提供不可读、格式变化和内容不一致时的安全回退；不得把一次例外扩展为目录扫描或任意文件访问。
- 已批准的 Codex 例外：ChatGPT/Codex 切换器可以只读 `~/.codex/.codex-global-state.json` 的 `project-order`、`thread-project-assignments`、`projectless-thread-ids` 与侧边栏项目展开状态，并只读 `~/.codex/state_5.sqlite` 的线程 `id`、`title`、`name`、`archived`、`preview` 与 `recency_at_ms`，用于还原当前 `Command + 1`～`Command + 9` 对应的最多 9 个对话标题。不得读取或记录项目路径、会话正文、预览内容、账号、Token 或其他字段；不得修改这些文件；解析失败时必须回退到辅助功能结果。
- 未在本节登记的跨 App 内部文件、数据库、沙盒容器、历史记录、私有运行库、私有协议或未公开数据格式仍禁止读取、解析、解密、复制或依赖；禁止通过独立 Helper、动态库或其他绕过方式扩大访问范围。
- 创建或更新 PR 前必须逐项检查登记范围、日志脱敏、只读属性与回退路径。自动化测试、模拟数据和签名包不能替代真实版本兼容性与隐私边界验收。
