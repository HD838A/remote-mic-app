...
从遥控页右上角可以重新发现 Mac 并进入 Mac App 信息页。该页面保持顶部导航固定，信息内容支持滚动，展示连接详情与检查结果、实体麦克风遥控器建议、GitHub 最新版下载链接，并支持通过系统分享（包括 AirDrop）发送链接、复制链接和导出脱敏诊断日志。iPhone 只作为临时应急方案，日常仍推荐使用按键反馈更清楚、更稳定的实体麦克风遥控器。
