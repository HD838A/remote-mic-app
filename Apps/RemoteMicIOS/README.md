...
当前版本提供方向、确定、返回、主页、菜单、TV、关机、音量和语音等核心遥控交互，并为每个按键提供明显的震动反馈。语音按钮支持点按开始、再次点按停止，也支持按住开始、松手停止。连接支持按钮事件能力的 Mac 后，全部可配置按键与实体遥控器一致支持单击、双击和长按自定义动作；旧版 Mac 继续回退到单击命令。页面使用固定单屏布局，不支持滚动。

中间六个按钮使用已经确认的固定布局：第一行依次为“返回 / 菜单 / 音量+”，第二行依次为“主页 / TV / 音量-”。除非产品需求明确变更，否则开发和重构不得调整该顺序。

iOS App 会优先通过普通局域网 Bonjour 发现附近运行新版无线麦的 Mac，3 秒无结果时自动切换到点对点发现，并保留有限重试与明确的 Wi-Fi / 重启恢复提示。发现后会显示真实 Mac 名称，并复用 Mac 端现有按键映射和虚拟音频输出。首次连接时，双方显示同一个两位校验码；用户在 Mac 明确允许后，按键和 16kHz 单声道麦克风音频通过安全连接传输。
...
从遥控页右上角可以重新发现 Mac 并进入 Mac App 信息页。该页面保持顶部导航固定，信息内容支持滚动，展示连接详情与检查结果、实体麦克风遥控器建议、GitHub 最新版下载链接，并支持通过系统分享（包括 AirDrop）发送链接、复制链接和导出 `.log` 脱敏诊断日志。日志包含网络路径、接口类型、Bonjour 浏览代次与结果变化、重试、连接路径与耗时、会话阶段和麦克风启动阶段，不包含音频、按键内容、设备名称、IP、校验码或密钥。iPhone 只作为临时应急方案，日常仍推荐使用按键反馈更清楚、更稳定的实体麦克风遥控器。
...
GitHub Actions 的 `iOS CI` 仅监听 `Apps/RemoteMicIOS/**` 和工作流自身，负责校验生成工程、构建 iOS App 并运行 iOS 单元测试，不处理 Mac 构建或发布。
