...
The packaged ``RemoteMicRC003.exe --bridge`` process is a long-lived owner of
BLE and audio resources. Starting a second instance in the same Windows logon
session would race both instances over those resources, so at most one bridge
instance may hold them at a time.
...
``CloseHandle``, ``MessageBoxW``) declares explicit ``argtypes`` and
``restype`` using ``ctypes.wintypes`` before invocation. ``wintypes.HANDLE``
is pointer-sized on every host, so a 64-bit handle is never truncated by an
unprototyped bare-integer default.
...
Testability: the real Win32 calls are isolated in module-level ``_real_*``
functions and injectable through ``BridgeInstanceGuard``. This lets
tests/test_single_instance.py exercise acquire, duplicate, release and cleanup
deterministically on any OS without ``ctypes.windll``. Only the real functions
call ``_require_windows()``; injected fakes remain platform-independent.
"""
