...
The same area also offers a **Capture audio from the remote microphone** switch, on by default. Turning it off makes the voice key a pure trigger — holding it only emits the chosen trigger key and never captures or routes the remote's audio, so the dictation tool can listen on a microphone the Mac already has (such as an external one). Fn tap (Typeless) is timed by the remote audio draining, so it pauses while capture is off and resumes when capture is back on.
