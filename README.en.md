...
**Local Karaoke** under **Connection & Voice** mixes audio actually captured by the Bluetooth remote directly into the Mac's current default physical sound output, so it can be monitored while music plays. Use the in-page button to turn it on or off. No physical button is assigned by default; under **Buttons**, you can assign **Toggle Local Karaoke** to any button's click, double-click, or long-press. That gesture toggles routing only: while the mode is on, hold the remote voice button to monitor locally, release it to end that stream, and hold it again to start the next one automatically. It is off by default, lasts only for the current app session, and turns off automatically when the Mac locks or sleeps or the app quits. Headphones are recommended to prevent speaker feedback.
