...
**Local Karaoke** under **Connection & Voice** mixes the Bluetooth remote microphone directly into the Mac's current default physical sound output, so it can be monitored while music plays. Use the in-page button to turn it on or off. No physical button is assigned by default; under **Buttons**, you can assign **Toggle Local Karaoke** to any button's click, double-click, or long-press. That gesture only toggles the mode: once enabled, monitoring continues without holding the voice button or pressing anything else, until the action is performed again or the in-page button turns it off. It is off by default, lasts only for the current app session, and turns off automatically when the Mac locks or sleeps or the app quits. Headphones are recommended to prevent speaker feedback.

When enabled, the app first switches the ATVV runtime capabilities to On-Request and then asks the remote to open its microphone. Hands-free monitoring is confirmed only when real PCM arrives on a host `session=0` stream, which is kept alive every eight seconds. If the remote never starts or returns an empty stream, the app closes it and retries at a bounded interval; it does not ask you to hold the voice button and cannot mistake physical voice-button audio for success. Turning the mode off restores standard PTT / HTT capabilities. This protocol path still requires RC001 / RC003 hardware confirmation. Karaoke audio is not sent to the virtual microphone, does not trigger Fn, and is neither saved nor transcribed.
