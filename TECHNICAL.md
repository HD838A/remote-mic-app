...
本文面向开发、审计和发布人员，描述 `1.0.0 (11)` 对应代码的实现、构建和发布约束。普通用户请阅读 [README.md](README.md)。
...
| 模块 | 主要职责 |
| --- | --- |
| `RemoteMicApp.swift` | AppKit 生命周期、菜单栏图标、左键设置窗口、右键菜单和关于面板 |
...
`scripts/test.sh` 运行 36 项协议/策略自检并编译完整应用。当前 Swift Testing 测试为 47 项，覆盖 ATVV、蓝牙生命周期、音频设备策略、按键、权限、Fn 映射和测试音。
...
- `dist/安装无线麦.pkg`；
- `dist/卸载无线麦.pkg`；
- `dist/Remote-Mic-1.0.0.dmg`；
- `dist/Remote-Mic-1.0.0.dmg.sha256`。
