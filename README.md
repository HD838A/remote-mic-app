...
“连接与语音”的“本地卡拉 OK”可把蓝牙遥控器的麦克风声音直接混入当前 Mac 的默认物理声音输出，播放音乐时即可本机返听。可以点击页面按钮开启和关闭；实体按键默认不绑定，需要时可在“按键”页面把任意按键的单击、双击或长按设为“切换本地卡拉 OK”。模式默认关闭、只在当前 App 运行期间生效，锁屏、睡眠或退出 App 会自动关闭；建议佩戴耳机，避免扬声器回授啸叫。

开启后 App 会先尝试由 Mac 主动保持蓝牙麦克风流。如果当前遥控器固件只回复开始状态、却不提供实际 PCM，1.5 秒后会自动关闭空流并在页面明确提示“按住语音键返听”；此时仍可按住语音键唱歌并从本机听到声音，但不能把该固件描述为免按键持续麦克风。卡拉 OK 音频不写入虚拟麦克风、不触发 Fn、不保存也不转写；关闭模式后原有语音输入路径保持不变。
