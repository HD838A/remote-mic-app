...
可选的“短按语音键定位输入框”默认关闭。开启后，短按只在当前最前面 App 中定位最可能的聊天输入框，长按继续使用上方选定的 Fn/左 Command/右 Command 语音触发键。通用路径使用 macOS Accessibility；微信不暴露聊天输入框，因此只在微信主窗口内使用有尺寸门禁的窗口相对点击。请关闭目标语音工具自己的短按启动功能；该选项与“语音键模拟 Fn 点按”互斥。
...
“连接与语音”的“本地卡拉 OK”可把蓝牙遥控器实际拾取到的声音直接送到单独选择的本机物理声音输出，播放音乐时即可返听。卡拉 OK 输出与上方普通“语音输出”完全独立：默认跟随系统声音输出，也可单独选择耳机、扬声器或外接声卡，不会修改普通语音绑定。指定设备暂时不可用时只临时回退到当前系统物理输出，虚拟音频设备不会作为卡拉 OK 播放目标。

可以点击页面按钮开启和关闭；实体按键默认不绑定，需要时可在“按键”页面把任意按键的单击、双击或长按设为“切换本地卡拉 OK”。映射手势只负责切换路由：开启前，无线麦会临时停用遥控器语音键对应的 Fn / Command 触发，防止同时启动系统输入法语音识别；无法安全停用时不会开启返听。开启后按住遥控器语音键即可从所选本机输出返听，松开后结束当前语音流；关闭返听后恢复用户原来的语音键模式。卡拉 OK 默认关闭、只在当前 App 运行期间生效，锁屏、睡眠或退出 App 会自动关闭；建议佩戴耳机，避免扬声器回授啸叫。

RC003 真机日志确认：Mac 主动 `MIC_OPEN` 只得到没有 PCM 的空流，只有按住实体语音键时遥控器才真正发送音频。因此卡拉 OK 模式不再主动开麦、重试或发送 `MIC_EXTEND`，而是保留遥控器的标准 PTT / HTT 行为，并把模式开启期间由目标遥控器产生的真实 PCM 路由到物理输出。卡拉 OK 音频不写入虚拟麦克风、不触发 Fn、不保存也不转写；模式关闭后，普通语音继续走原有虚拟麦克风链路。
...
每个普通按键都可以设置单击动作，并可额外设置双击和长按动作。动作支持键盘操作、系统音量、播放控制、切换本地卡拉 OK、打开当前 Mac 已安装的常用应用，以及任意自定义键盘快捷键。自定义快捷键既可以直接选择复制、粘贴、聚焦搜索等常用组合，也可以在页面内的标准键盘选择组合键、主键、F1–F20、导航键、数字键盘或单独的左右修饰键；仍保留真实键盘录入入口。
...
语音键始终用于语音输入，不参与普通按键映射；触发方式可在语音键区域选择 Fn/地球键、左 Command 或右 Command 长按，也不会默认触发本地卡拉 OK。
