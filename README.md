...
在“按键映射”页的“语音键”区域可以选择语音触发方式：默认的 Fn/地球键，或左 Command、右 Command、右 Option、右 Shift 长按。Fn/地球键保持旧版本行为；修饰键模式需要无线麦的“辅助功能”权限，并会在语音开始时按下所选修饰键、结束时释放。目标语音应用必须支持对应按键的单侧长按；许多应用会把左右侧合并为通用修饰键，需在目标应用中实际测试。修饰键长按期间同时按其他键可能触发对应的系统快捷键。
...
语音键始终用于语音输入，不参与普通按键映射；触发方式可在语音键区域选择 Fn/地球键、左/右 Command、右 Option 或右 Shift 长按。
