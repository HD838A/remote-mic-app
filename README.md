...
如果希望键盘触发语音时继续使用 USB 或 Mac 麦克风，可在同一页面开启“混入电脑麦克风”，选择物理麦克风，并保持语音工具仍使用 `MiRemoteV 2ch`。无线麦会把物理麦克风与遥控器声音送入同一个虚拟麦克风；关闭开关后立即停止读取物理麦克风。该功能需要 macOS 麦克风权限，且不会上传或保存声音。
