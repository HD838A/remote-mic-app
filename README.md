...
“连接与语音”的“本地卡拉 OK”可把蓝牙遥控器的麦克风声音直接混入当前 Mac 的默认物理声音输出，播放音乐时即可本机返听。可以点击页面按钮开启和关闭；实体按键默认不绑定，需要时可在“按键”页面把任意按键的单击、双击或长按设为“切换本地卡拉 OK”。映射手势只负责切换：开启后无需再按住语音键或任何其他键，持续返听直到再次执行该手势或点击关闭。模式默认关闭、只在当前 App 运行期间生效，锁屏、睡眠或退出 App 会自动关闭；建议佩戴耳机，避免扬声器回授啸叫。

开启后 App 会先把 ATVV 运行时能力切换为 On-Request，再由 Mac 主动打开蓝牙麦克风；只有 `session=0` 的主机会话收到真实 PCM 后才会显示持续返听，并每 8 秒发送保活。遥控器没有开始传输或只返回空流时，App 会关闭空会话并按有界间隔自动重试，不会要求改为按住语音键，也不会把实体语音键产生的音频误报为成功。关闭模式后会恢复标准 PTT / HTT 能力。该协议路径仍需 RC001 / RC003 真机确认固件支持；卡拉 OK 音频不写入虚拟麦克风、不触发 Fn、不保存也不转写。
...
语音键始终用于语音输入和 Fn 功能，不参与普通按键映射，也不会默认触发本地卡拉 OK。
