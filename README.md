...
按住说，松开停。开会不发言、听歌不暂停，也能把文字送进正在使用的 App。无线麦不修改 Mac 的默认输入或输出设备。
...
如果保留千问输入法默认的“右 Command”语音快捷键，可在“连接与语音”中开启“千问兼容模式”。无线麦用 RC003 的硬件 HID 事件启动千问。请先关闭千问的“短按也能开始输入”；此后在当前最前面的 App 里，点按一次语音键会尝试把光标放到最可能的聊天输入框，再长按即可说话。SayAll 不会切换 App 或关闭弹窗；通用路径不使用坐标，但微信不向 macOS 暴露聊天输入框，因此只在微信主窗口内使用窗口相对位置作为短按聚焦降级。松开语音键并排空尾音后，应用短暂屏蔽 F5、释放 Command，并发送一个无系统动作的 F20 让千问确认当前语音，再恢复右 Command 映射供下次使用。无线麦会自动选择 `MiRemoteV 2ch` 作为语音输出；还需在千问中把麦克风明确选择为 `MiRemoteV 2ch`，不使用“系统默认”。

遥控器仍然必须按住说话、松开停止。该模式与“语音键模拟 Fn 点按”互斥；开启任一模式会关闭另一模式。
...
每个普通按键都可以设置单击动作，并可额外设置双击和长按动作。动作支持键盘操作、系统音量、播放控制、打开当前 Mac 已安装的常用应用，以及任意自定义键盘快捷键。自定义快捷键既可以直接选择复制、粘贴、聚焦搜索等常用组合，也可以在页面内的标准键盘选择组合键、主键、F1–F20、导航键、数字键盘或单独的左右修饰键；仍保留真实键盘录入入口。
...
无线麦不会上传或保存语音，不修改系统默认输入、输出设备，也不会在日志中记录语音内容、蓝牙地址或外设标识。
