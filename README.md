...
如果希望键盘触发语音时继续使用 USB 或 Mac 麦克风，可在同一页面开启“混入电脑麦克风”，选择物理麦克风，并保持语音工具仍使用 `MiRemoteV 2ch`。无线麦平时不读取物理麦克风；第一次按右 Command 启动语音输入时开始送入 `MiRemoteV 2ch`，再次按右 Command 结束时立即释放。遥控器仍只在按住语音键时送入声音。该功能需要 macOS 麦克风与输入监控权限，且不会上传或保存声音。
...
不同遥控器的收音方式不同：小米遥控器只能按住收音；Chromecase 语音遥控器既支持按住，也支持「按一下开始、再按一下结束」；Siri Remote 另有触摸面。Mac 端的语音工具同样分两类：有的按住说话，有的按一次切换。**两边不是同一类时会出现「按一下结束没生效、要再按一下」或「按下没说几句就结束」。**

各款遥控器、各款输入工具的能力，以及每种组合该用哪种模式（包括「语音键模拟 Fn 点按」要不要开），见[遥控器与输入工具能力矩阵](remote/遥控器与输入工具能力矩阵.md)。
