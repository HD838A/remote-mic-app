...
按住说，松开停。开会不发言、听歌不暂停，也能把文字送进正在使用的 App。无线麦默认不修改 Mac 的默认输入或输出设备；只有用户主动开启千问兼容模式时临时管理默认输入。
...
如果保留千问输入法默认的“右 Command”语音快捷键，可在“连接与语音”中开启“千问兼容模式”。无线麦会把 RC003 语音键映射为右 Command，自动选择 `MiRemoteV 2ch`，并仅在遥控器连接、音频输出可用时临时将系统默认麦克风切换到该设备；关闭模式、断开遥控器或退出无线麦时恢复此前的默认输入。用户在模式开启期间手动选择其他系统默认麦克风后，无线麦不会覆盖该选择。
...
无线麦不会上传或保存语音，默认不修改系统默认输入、输出设备，也不会在日志中记录语音内容、蓝牙地址或外设标识。只有用户主动开启“千问兼容模式”时，应用才会在遥控器可用期间临时管理系统默认输入，并在关闭、断连或退出时恢复。
