...
- 通过 IOHID 读取 RC003 原始按键报告，提供返回、主页、菜单、TV、音量等 macOS 动作映射，并可把普通按键自定义为打开预置 APP。
...
点击或按下遥控器实物图上的普通按键即可定位映射项。每个普通按键除了可以映射为键盘和系统动作，还可以自定义为打开 Codex、Claude、cmux、微信、Cursor、Xcode、Slack、企业微信、网易云音乐、Chrome、Safari 或 Zed。目标 APP 已运行时会切换到前台，未运行时会自动启动，不会创建重复实例或额外窗口。

按键映射修改后自动保存，也可一键恢复默认映射。动作选择器只显示当前 Mac 已安装的预置 APP；如果已绑定的 APP 后来被卸载，该按键会保留映射并显示“未安装”。恢复默认映射后，TV 键仍为 Command-Tab。语音键保留固定的设备专属核心行为，不参与普通按键映射。
...
设置页使用保持原始比例的 RC003 实物图，点击或按下真实实体按键会定位到右侧映射项；实物没有独立静音键，因此界面不再显示虚构的静音实体键。每个普通按键都可以改成预置键盘/系统动作、打开预置 APP 或禁用，选择后自动保存，也可一键恢复默认映射。语音键执行固定的“设备专属 F5→Fn 硬件映射 + ATVV 语音桥接”核心动作，不参与普通按键映射。
