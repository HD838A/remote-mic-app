...
“连接与语音”的“本地卡拉 OK”可把蓝牙遥控器实际拾取到的声音直接混入当前 Mac 的默认物理声音输出，播放音乐时即可本机返听。可以点击页面按钮开启和关闭；实体按键默认不绑定，需要时可在“按键”页面把任意按键的单击、双击或长按设为“切换本地卡拉 OK”。映射手势只负责切换路由：开启后，按住遥控器语音键即可自动从物理输出返听，松开后结束当前语音流；再次按住会自动开始下一段返听。模式默认关闭、只在当前 App 运行期间生效，锁屏、睡眠或退出 App 会自动关闭；建议佩戴耳机，避免扬声器回授啸叫。
