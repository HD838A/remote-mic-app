...
如果保留千问输入法默认的“右 Command”语音快捷键，可在“连接与语音”中开启“千问兼容模式”。无线麦用 RC003 的硬件 HID 事件启动千问。请先关闭千问的“短按也能开始输入”；此后在当前最前面的 App 里，点按一次语音键会尝试把光标放到最可能的聊天输入框，再长按即可说话。SayAll 不会切换 App、关闭弹窗或使用固定屏幕坐标；当前 App 有多个相似输入框时，仍可能需要针对该 App 调整识别规则。松开语音键并排空尾音后，应用短暂屏蔽 F5、释放 Command，并发送一个无系统动作的 F20 让千问确认当前语音，再恢复右 Command 映射供下次使用。无线麦会自动选择 `MiRemoteV 2ch` 作为语音输出；还需在千问中把麦克风明确选择为 `MiRemoteV 2ch`，不使用“系统默认”。
