...
如果保留千问输入法默认的“右 Command”语音快捷键，可在“连接与语音”中开启“千问兼容模式”。无线麦会屏蔽 RC003 原本的 F5，按住语音键时单独按下右 Command，松开并排空尾音后再松开右 Command，从而避免触发 macOS 的 Command-F5 / VoiceOver。无线麦会自动选择 `MiRemoteV 2ch` 作为语音输出；还需在千问中把麦克风明确选择为 `MiRemoteV 2ch`，不使用“系统默认”。
