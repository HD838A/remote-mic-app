...
“语音键”区域还提供“用遥控器自带麦克风收音”开关，默认开启。关闭后语音键变成纯触发器——按住只发送所选触发键，不再采集或路由遥控器音频，语音输入改用 Mac 上已有的麦克风（如外接麦克风），适合嘈杂环境或已有专业麦克风的场景。Fn 点按（Typeless）依赖遥控器音频排空计时，关闭期间自动暂停，重新开启后恢复。
