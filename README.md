...
如果保留千问输入法默认的“右 Command”语音快捷键，可在“连接与语音”中开启“千问兼容模式”。无线麦用 RC003 的硬件 HID 事件启动千问；仅当 Codex 是最前面的 App、但编辑框没有光标时，第一次点按语音键会取消这次语音并等待 Codex 编辑框获得光标，准备好后再长按即可说话。其他 App 保持现有行为，不自动切换或处理页面弹层。松开语音键并排空尾音后，应用短暂屏蔽 F5、释放 Command，并发送一个无系统动作的 F20 让千问确认当前语音，再恢复右 Command 映射供下次使用。这样避免遗留 Command-F5 / VoiceOver 或全局 Command 状态。无线麦会自动选择 `MiRemoteV 2ch` 作为语音输出；还需在千问中把麦克风明确选择为 `MiRemoteV 2ch`，不使用“系统默认”。
