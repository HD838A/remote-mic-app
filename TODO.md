...
- [ ] Windows 版本
  - 当前范围只包含小米 RC003；已按“选择性移植 PR 核心”落地 WinRT BLE、ATVV/ADPCM、显式音频端点、重连和单实例保护，并加入普通权限 Raw Input/SendInput 按键映射、单击/双击/长按、自定义快捷键和 PySide6 四页设置界面。麦克风键固定保留语音核心行为；不包含 DJI、Frida、WUDFHost 注入、豆包进程附加、完整 HID tap 或 VB-CABLE 分发。
  ...
  - 包含 PySide6 四页界面与完整按键映射链路的 unsigned Windows Actions 已在 [run 30730044548](https://github.com/HD838A/remote-mic-app/actions/runs/30730044548) 成功完成 180 项测试、PyInstaller、EXE `--dry-run`、Inno Setup 和 artifact 扫描。此条仍未完成，尚需免费自签 tag、真实 Windows + RC003 + 用户自行安装的 VB-CABLE 端到端验收，以及 13 个页面映射项的逐键可检测性、原始动作泄漏和 UIPI 限制验证。
