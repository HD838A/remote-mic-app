...
- v1.4.12 已作为 macOS 14 候选 pre-release 发布。
- v1.4.13 已晋升为正式版，并成为 GitHub Latest；正式产物的 App、驱动和 Sparkle appcast 最低版本均为 macOS 14.0，架构为 `arm64`。
- 仓库目前没有记录真实 macOS 14 Apple Silicon 机器完成全部运行时验收的证据。正式发布事实不能替代下方真机验证，剩余项目继续在 `TODO.md` 单独跟踪。
...
原计划将真实硬件验证作为正式晋升门禁；实际 v1.4.13 已在仓库没有完整真机记录的情况下晋升正式版。发布机上的交叉编译、签名和包体检查仍不能单独证明所有 macOS 14 运行时行为，因此上述真机项目必须继续补齐并留存证据。
