...
7. **当前所有实施资源只投入 RC003 Windows 产品。** 已落地不依赖进程注入的麦克风主链路、普通权限按键映射、PySide6 四页设置界面、独立打包和免费自签脚本；完整隐藏键 HID tap 仍不属于普通权限版本。Apple Remote 不参与当前技术选型，也不作为 Windows 首版验收项。
...
| 13 个实体按键 | 当前页面提供 13 个映射项，标准 Raw Input 已接入 | 可检测键集合与原始动作泄漏仍需逐键真机验收；完整体验可能依赖 HID tap |
| 单击/双击/长按 | 已落地按钮手势模块与映射逻辑 | 仍需逐键真机验收 |
| 自定义组合快捷键 | 已落地 | 受 SendInput/UIPI 约束 |
| 打开应用 | 已落地 Windows 动作执行 | 应用安装位置和聚焦策略仍需真机验收 |
| 蓝牙自动重连 | 有 connection supervisor | 需长时间稳定性验收 |
| 设置页与诊断 | 已重写为 PySide6 Widgets 四页侧边栏，与 Mac 信息架构一致 | 目前主要为中文，视觉不承诺逐像素相同 |
...
- 保留 GPL 来源和上游归属；
- 已从当前范围删除 Frida 和 VB-CABLE 分发；保留并声明 PortAudio/sounddevice、NumPy、PyWinRT、PySide6-Essentials、PyInstaller 和 Inno Setup；
...
### 阶段 2：安全的普通按键（代码已落地，真机待验收）

- 当前只使用 Raw Input 和 SendInput；
- 列出无需管理员权限即可稳定识别的按键；
