...
本文规定无线麦SayAll.app 的运行日志、用户可复制诊断，以及由测试程序产生的日志证据应如何设计与输出。目标是在不记录用户敏感信息的前提下，仅凭日志即可快速还原一次操作经过、确定失败边界，并区分确定事实与推测原因。

本规范不禁止发布和工程审计记录公开资产文件名、仓库相对路径、公开 Tag/Run URL、版本、摘要或 artifact ID；这些内容属于可审计的构建与发布身份，按 [`RELEASING.md`](RELEASING.md) 和对应测试手册保存。无论何种证据，都不得记录用户目录、用户选择的私有文件名、凭据、用户内容或可识别设备身份。
...
任何运行日志、统一日志、用户可复制诊断、崩溃附加信息和由测试程序产生的运行日志都不得包含：
...
- 用户目录、用户选择的完整文件路径、私有文件名、文档名、窗口标题或工作区名称。固定的公开资产名、仓库相对路径和公开 URL 只能出现在工程或发布审计证据中，不能进入用户运行日志或复制诊断。
