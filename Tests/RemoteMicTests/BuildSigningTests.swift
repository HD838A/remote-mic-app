...
struct BuildSigningTests {
    ...
    @Test func previewBranchPushReusesExactParentMainProofWithoutRepeatingBuilds() throws {
        ...
        #expect(workflowSource.contains("./scripts/verify-release-ready-main-ci.sh"))
        #expect(workflowSource.contains("timeout-minutes: 3"))
        #expect(workflowSource.contains("reuse_parent_main_ci == 'true'"))
        #expect(workflowSource.contains("reuse_parent_main_ci != 'true'"))
        #expect(workflowSource.contains("Run full candidate validation when reuse is ineligible"))
    ...
    @Test func optimizedReleasePipelineHasExecutableRegressionCoverage() throws {
        ...
        #expect(releaseWorkflowSource.contains("TOTAL_SLO_SECONDS: 1800"))
        #expect(releaseWorkflowSource.contains("READY_SLO_SECONDS: 900"))
        #expect(releaseWorkflowSource.contains("release_ready_at:"))
        #expect(releaseWorkflowSource.contains("PREVIEW_PUBLISHED_SLO_SECONDS: 840"))
    ...
    @Test func stablePromotionRequiresMainAndCandidateProvenance() throws {
        ...
        #expect(promotionWorkflow.contains("workflow_run reconciliation has no authoritative user-request timestamp."))
        ...
        #expect(promotionWorkflow.contains("Enforce 30-minute user-wall stable SLO"))
        #expect(promotionWorkflow.contains("SLO_SECONDS: 1800"))
        #expect(promotionWorkflow.contains("STABLE_COMPLETION_SLO_SECONDS: 1740"))
