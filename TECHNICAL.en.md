...
| RemoteVoiceFunctionMapper.swift | RC003 voice-button F5 mapping to Fn/Globe, Qianwen hardware Right Command, or neutralization for software Command modes, with restoration |
...
The opt-in Qianwen compatibility mode fully maps physical F5 to Right GUI / Right Command (usage page `0x07`, usage `0xE7`) because Qianwen's global shortcut monitor ignores ordinary `CGEvent` simulation. Qianwen's short-press voice option must be off. When ATVV starts without a safe editable destination, SayAll neutralizes F5, releases Right Command, drops that audio attempt, and calls `KeyboardInjector.focusFrontmostComposer` for the current frontmost app. Right Command is rearmed after focus succeeds, so the next hold starts voice input. A session shorter than 550 ms takes the same focus path, covering apps whose Accessibility tree reports an editable destination without a visible cursor. This path never switches apps, dismisses dialogs, or uses screen coordinates. After ATVV `STREAM_STOP` and queued audio drain, SayAll again neutralizes F5, posts a Right Command key-up, sends a harmless F20 event that makes Qianwen confirm, and rearms the mapping 150 ms later. Disconnect, mode disable, and app exit cancel pending work and release Command. The mode selects `MiRemoteV 2ch` as SayAll's output, while Qianwen must explicitly select the same microphone. It is mutually exclusive with Fn tap mode. Legacy configurations without `qianwenVoiceModeEnabled` treat it as off.
