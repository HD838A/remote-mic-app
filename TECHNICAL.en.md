...
The opt-in Qianwen compatibility mode fully maps physical F5 to Right GUI / Right Command (usage page `0x07`, usage `0xE7`) because Qianwen's global shortcut monitor ignores ordinary `CGEvent` simulation. When ATVV starts, SayAll intercepts the attempt only if Codex is frontmost and `VoiceInputDestinationSnapshot.system()` has no safe editable focus: it temporarily maps F5 to usage `0`, releases Right Command, and reuses `VoiceInputDestinationCoordinator` to focus the composer in the current Codex window. It rearms Right Command when ready, so the next hold starts voice input. Other apps do not enter this automatic-focus branch. After ATVV `STREAM_STOP` and queued audio drain, SayAll again neutralizes F5, posts a Right Command key-up, sends a harmless F20 event that makes Qianwen confirm, and rearms the mapping 150 ms later. Disconnect, mode disable, and app exit cancel pending work and release Command. The mode selects `MiRemoteV 2ch` as SayAll's output, while Qianwen must explicitly select the same microphone. It is mutually exclusive with Fn tap mode. Legacy configurations without `qianwenVoiceModeEnabled` treat it as off.
