...
The opt-in macOS Dictation mode is mutually exclusive with Typeless mode. It reuses F5 neutralization, pre-roll, and audio draining, then sends two Left Control taps when a physical Xiaomi RC003 voice stream starts and two more after the audio tail drains. Each press lasts 60 milliseconds, with an 80-millisecond gap. After the opening double-tap, the controller writes 4,000 silent samples to the 16 kHz audio queue before playing pre-roll, giving Dictation about 250 milliseconds to start listening. The system Dictation shortcut must be set to “Press Control Key Twice.” On a first connection whose profile is still unknown, only this mode waits for the parallel 2A24 model read to finish before Ready; disabling the mode releases that wait without cancelling the in-flight read. RC001, unidentified models, iPhone, Apple Watch, and Web voice paths do not send the Control sequence.

These tap modes only adapt the trigger semantics seen by the target app. The RC003 must still be physically held to capture audio, and neither mode provides continuous recording or independent transcription. Configuration import/export includes optional `voiceKeyMode`, `voiceFnTapModeEnabled`, and `voiceMacOSDictationModeEnabled`; legacy configurations keep new behavior off. Mode changes, disconnects, and app exit release any synthetic key still held and restore the managed source usages while preserving unrelated runtime changes. Runtime Dictation cleanup keeps the normal Control timing; process exit only performs synchronous best-effort release and compensation, which is not evidence that Dictation responded.
...
`scripts/test.sh` runs protocol and policy self-tests and compiles the full app. Swift Testing covers ATVV, Bluetooth lifecycle, audio-device policy, button mapping, permissions, configuration compatibility, Fn mapping, the Typeless and macOS Dictation session lifecycles, pre-roll, audio draining, and test-tone behavior.
